...
if __name__ == '__main__':
    ...
    df1 = df[['s_rsa', 's_up', 's_down', 's_phi', 's_psi', 's_a1', 's_a2', 's_a3', 's_a4', 's_a5',
        't_rsa', 't_up', 't_down', 't_phi', 't_psi', 't_a1', 't_a2', 't_a3', 't_a4', 't_a5']]
    
    features = ['s_rsa', 's_up', 's_down', 's_phi', 's_psi', 's_a1', 's_a2', 's_a3', 's_a4', 's_a5', 
        't_rsa', 't_up', 't_down', 't_phi', 't_psi', 't_a1', 't_a2', 't_a3', 't_a4', 't_a5']

    #replace NaN values with average value of respectively column  
    df1 = df1.apply(pd.to_numeric, errors='coerce')
    for i in features:
        df1[i]=df1[i].fillna(df1[i].mean(axis=0))
    for i in features:
        df1[i]=df1[i].fillna(0)
    #check if there are NaN values
    # value = df1.isnull().sum()
    # print(value)
    
    #create an array for the predicted values
    predicted_interaction = []
    for i in range(len(df1['s_up'])):
        predicted_interaction.append(rf.predict([df1.iloc[i].values]))
    
    #create a new column with the predicted values 
    df1['interaction']=predicted_interaction

    df1.to_csv("{}/{}.tsv".format(args.out_dir, pdb_id), sep="\t", index=False)
